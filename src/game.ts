...
export function applyMove(state: GameState, move: MoveChoice | null): GameState {
  ...
  if (move) {
    ...
    log.push(`${currentPlayer.name} 当前无可钓组合，翻开了 1 张公共牌`);
  }

  if (deck.length > 0) {
    const publicDraw = deck[0];
    if (publicDraw) {
      publicCards.push(publicDraw);
      deck = deck.slice(1);
      if (capturedThisTurn) {
        log.push('公共区域补了 1 张牌');
      } else if (!move) {
        log.push('系统向公共区域新增了 1 张牌');
      } else {
        log.push('本回合结束，公共区域照常新增了 1 张牌');
      }
    }
  }
