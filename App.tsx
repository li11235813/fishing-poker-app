...
5. 收牌成功后：本回合立即结束；该玩家手牌补回到 5 张；公共区只补 1 张；切换对手。
6. 若当前没有任何可钓组合，并且牌堆还有牌：可翻开 1 张公共牌，然后切换对手。
7. 超时 30 秒未操作：判定本回合失败，公共区补 1 张，然后切换对手。
8. 当牌堆发完后，不再补齐 5 张；玩家每回合必须打出 1 张手牌：能钓就钓，不能钓就打到公共区。
9. 计分：黑桃 4 分、梅花 3 分、红桃 2 分、方块 1 分。
10. 全部手牌打完后，按已收牌总分判定胜负。`;
...
function CardView({ card, selected, onClick }: { card: Card; selected?: boolean; onClick?: () => void }) {
  return (
    <button type="button" className={`${cardClass(card)} ${selected ? 'selected' : ''}`.trim()} onClick={onClick}>
      <span>{card.rank}</span>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const turnEndsAt = game.turnEndsAt;
  ...
  useEffect(() => {
    if (!game || game.status !== 'playing' || secondsLeft > 0 || !isMyTurn || !room || !playerId) return;

    playMove(room.roomCode, playerId, null)
      .then((data) => {
        setRoom(data.room);
  ...
  }, [secondsLeft, game, isMyTurn, playerId, room]);
  ...
  async function submitMove() {
    if (!room || !playerId || !selectedHand) return;
    setBusy(true);
    setError(null);
    try {
        ...
        <main className="board">
          ...
          <section className="panel">
            ...
            <div style={{ marginTop: 12 }}>
              <button onClick={submitMove} disabled={!selectedHand || !isMyTurn || busy}>出这张牌</button>
